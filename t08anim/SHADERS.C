 ...
 * RETURNS: None.
 */
static VOID SA2_RndShaderLog( CHAR *Stage, CHAR *Text )
{
  FILE *F;

  if ((F = fopen("SA2{SHAD}30.LOG", "a")) != NULL)
  {
    fprintf(F, "%s: %s\n", Stage, Text);
    fclose(F);
  }
 ...
 */
static CHAR * SA2_RndShaderLoadTextFile( CHAR *FileName )
{
  FILE *F;
  CHAR *text;
  INT flen;
...
UINT SA2_RndShaderLoad( CHAR *FileNamePrefix )
{
  ...
  for (i = 0; isok && i < NS; i++)
  {
    ...
    {
      SA2_RndShaderLog(suff[i], "Error create shader");
      isok = FALSE;
      break;
    }
    /* Load text file */
    sprintf(buf, "%s.%s", FileNamePrefix, suff[i]);
    txt = SA2_RndShaderLoadTextFile(buf);
    if (txt == NULL)
    {
      SA2_RndShaderLog(suff[i], "No file");
      isok = FALSE;
      break;
    }
  ...
  return prg;
} /* End of 'SA2_RndShaderLoad' function */
...
VOID SA2_RndShaderFree( UINT Prg )
{
  ...
  glDeleteProgram(Prg);
} /* End of 'SA2_RndShaderFree' function */
